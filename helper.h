...
void print_result(int id, std::string text_id, int vec_size, int small_size, int first_time, int64_t peak_size, double access_avg, int snd_time, int big_size, double rank_avg, double select_avg, int s, int tau, int leaf_size, int64_t result) {
    std::cout << id << ",";
    std::cout << text_id << ",";
    std::cout << vec_size << ",";
    std::cout << small_size << ",";
...
//    }
    ...
    print_result(id, text_id, input.size(), cbt_size_no_rank(*cbt), ms_int1.count() , size, (double)elapsed/access_queries_.size(), ms_int2.count() + ms_int3.count(),cbt_size_rank(*cbt) ,(double)elapsed2/select_queries_.size(),(double)elapsed3/select_queries_.size(), 1, tau, max_leaf_size, result);
    delete cbt;
    return 0;
}
int run_bench_lpf_pruned_s(std::vector<uint8_t>& vec, std::string& text_id, std::vector<int>& access_queries_, std::vector<int>& select_queries_, std::vector<uint8_t>& select_c_, int s, int tau, int max_leaf_size) {
    int id = 6;
    auto wm = pasta::make_wm<pasta::BitVector>(vec.begin(), vec.end(),
                                               256);
    auto t01 = std::chrono::high_resolution_clock::now();
    BV_BlockTree_lpf_pruned<uint8_t, int64_t>*  bt = new BV_BlockTree_lpf_pruned<uint8_t, int64_t>(vec, tau, max_leaf_size,s,true, true, true);
    auto t02 = std::chrono::high_resolution_clock::now();
...
int run_bench_lpf_pruned(std::vector<uint8_t>& vec, std::string& text_id, std::vector<int>& access_queries_, std::vector<int>& select_queries_, std::vector<uint8_t>& select_c_, int s, int tau, int max_leaf_size) {
    ...
    BV_BlockTree_lpf_pruned<uint8_t, int32_t>*  bt = new BV_BlockTree_lpf_pruned<uint8_t, int32_t>(vec, tau, max_leaf_size,true, true, false);
    auto t02 = std::chrono::high_resolution_clock::now();
    auto small_size = bt->print_space_usage();
    bt->add_rank_support();
    auto t03 = std::chrono::high_resolution_clock::now();
...
int run_bench_lpf_theory(std::vector<uint8_t>& vec, std::string& text_id, std::vector<int>& access_queries_, std::vector<int>& select_queries_, std::vector<uint8_t>& select_c_, int s, int tau, int max_leaf_size) {
    int id = 2;
    auto t01 = std::chrono::high_resolution_clock::now();
    BV_BlockTree_fp_pruned<uint8_t, int32_t>*  bt = new BV_BlockTree_fp_pruned<uint8_t, int32_t>(vec, tau, max_leaf_size,s,256,
                                                                                                 true, true);
    auto t02 = std::chrono::high_resolution_clock::now();
    auto small_size = bt->print_space_usage();
    bt->add_rank_support();
...
int run_wavelt(std::vector<uint8_t>& vec, std::string& text_id, std::vector<int>& access_queries_, std::vector<int>& select_queries_, std::vector<uint8_t>& select_c_, int s, int tau, int max_leaf_size) {

    int id = 7;
//    BV_BlockTree_lpf_pruned<uint8_t, int32_t>*  bt = new BV_BlockTree_lpf_pruned<uint8_t, int32_t>(vec, 2, 4,1,true, true, true);
//    bt->add_rank_support();
    auto t01 = std::chrono::high_resolution_clock::now();
    auto wm = pasta::make_wm<pasta::BitVector>(vec.begin(), vec.end(),
                                               256);
    ...
    start = std::chrono::high_resolution_clock::now();
    for (int i = 0; i < select_c_.size(); i++) {
//        if (wm.rank(select_queries_[i],select_c_[i]) != bt->rank(select_c_[i],select_queries_[i])) std::cout << i << std::endl;
...
//        if (wm.rank(select_queries_[i],select_c_[i]) != bt->rank(select_c_[i],select_queries_[i])) std::cout << i << std::endl;
    ...
    print_result(id, text_id, vec.size(), 0, ms_int1.count(), 0,(double)elapsed/access_queries_.size(), ms_int2.count(), 0, (double)elapsed2/select_queries_.size(), (double)elapsed3/select_queries_.size() , s, tau, max_leaf_size, result );
    return 0;
}
int run_wavelt2(std::vector<uint8_t>& vec, std::string& text_id, std::vector<int>& access_queries_, std::vector<int>& select_queries_, std::vector<uint8_t>& select_c_, int s, int tau, int max_leaf_size) {

    int id = 8;
    auto t01 = std::chrono::high_resolution_clock::now();
    auto wm = pasta::make_wt<pasta::BitVector>(vec.begin(), vec.end(),
                               256);
    auto t02 = std::chrono::high_resolution_clock::now();
    auto t03 = std::chrono::high_resolution_clock::now();
    auto ms_int1 = std::chrono::duration_cast<std::chrono::milliseconds>(t02 - t01);
